...
VAULT="$HOME/Constellation"
GIT_ROOT="$HOME/Constellation"
SESSION_LOG="$VAULT/C - Signal Relay/Session Log.md"
DATETIME=$(date +"%Y-%m-%d %H:%M")

# Find changed vault .md files via git status
CHANGED_FILES=$(cd "$GIT_ROOT" && git status --short -- "*.md" 2>/dev/null \
  | grep -v "_index.md" \
  | grep -v "Daily/" \
  | grep -v "Weekly/" \
  | awk '{print $2}' \
...
DIFF=$(cd "$GIT_ROOT" && git diff HEAD -- "*.md" 2>/dev/null | head -150)
